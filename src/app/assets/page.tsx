...
export default function AssetsPage() {
  ...
  const fetchConditionHistory = useCallback(async (assetId: string) => {
    setHistoryLoading(true)
    try {
      const res = await fetch(`/api/assets/${assetId}/condition-history`)
      if (!res.ok) return
      const json = await res.json()
      setConditionHistory(json.data?.history || [])
    } catch {
      setConditionHistory([])
    } finally {
  ...
  const toggleHistory = (assetId: string) => {
    if (expandedAssetId === assetId) {
      setExpandedAssetId(null)
      setConditionHistory([])
    } else {
      setExpandedAssetId(assetId)
      fetchConditionHistory(assetId)
    }
  }
              ...
              <div key={asset.id}>
                ...
                {isExpanded && (
                  <div className="px-5 pb-4 pt-0">
                    <div className="ml-[52px] bg-ink-50 rounded-lg p-4">
                      <h4 className="text-[12px] font-semibold text-ink-700 mb-3">Condition History</h4>
                      {historyLoading ? (
                        <div className="flex items-center gap-2 py-2">
                          <Loader2 className="w-3.5 h-3.5 animate-spin text-ink-400" />
                          <span className="text-[11px] text-ink-400">Loading history...</span>
                        </div>
                      ) : conditionHistory.length === 0 ? (
                        <p className="text-[11px] text-ink-400">No condition changes recorded yet.</p>
                      ) : (
                        <div className="space-y-2">
                          {conditionHistory.map((entry) => (
                            <div key={entry.id} className="flex items-start gap-3 text-[11px]">
                              <span className="text-ink-300 whitespace-nowrap min-w-[100px]">
                              ...
                              {entry.notes && (
                                <span className="text-ink-500 truncate">{entry.notes}</span>
                              )}
                            </div>
                          ))}
                        </div>
